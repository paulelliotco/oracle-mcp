...
// Shared config (keep in sync with index.ts)
const VERSION = "0.1.0";
const DEFAULT_MODEL = process.env.ORACLE_MODEL || "o3";
...
const log = {
    debug: (...args) => LOG_LEVEL === "debug" && console.debug("[oracle-mcp:http]", ...args),
    info: (...args) => ["debug", "info"].includes(LOG_LEVEL) && console.log("[oracle-mcp:http]", ...args),
    warn: (...args) => console.warn("[oracle-mcp:http]", ...args),
    error: (...args) => console.error("[oracle-mcp:http]", ...args),
};
...
function isRetryableStatus(status) { return status === 429 || (status !== undefined && status >= 500 && status < 600); }
...
function buildServer() {
    const server = new McpServer({ name: "oracle-mcp", version: VERSION });
    function registerOracleTool(name) {
...
// Optional basic rate limiting for public endpoints (60 req/min per IP)
if (process.env.HTTP_RATE_LIMIT === "1") {
    const limiter = rateLimit({ windowMs: 60_000, max: 60, standardHeaders: true, legacyHeaders: false });
    app.use(limiter);
}
// Optional auth: require x-mcp-auth header if MCP_AUTH_TOKEN is set
app.use((req, res, next) => {
    const token = process.env.MCP_AUTH_TOKEN;
    if (token && req.header("x-mcp-auth") !== token) {
...
app.post("/mcp", async (req, res) => {
    ...
    if (!transport) {
        ...
        transport.onclose = () => { if (transport?.sessionId)
...
app.listen(PORT, () => {
    log.info(`oracle-mcp Streamable HTTP server listening on :${PORT}`);
});
